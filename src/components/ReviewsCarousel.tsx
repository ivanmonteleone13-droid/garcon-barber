      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-12 md:hidden">
          ...
          <div className="mt-6 flex items-center justify-center gap-4">
            <button type="button" onClick={() => setCurrent((c) => (c - 1 + total) % total)} className="rounded-full border border-white/20 p-2" aria-label="Föregående omdöme">←</button>
            <div className="flex gap-2" role="tablist" aria-label="Välj omdöme">
              {business.testimonials.map((_, i) => (
                <button key={i} type="button" role="tab" aria-selected={i === current} onClick={() => setCurrent(i)} className={`h-2 w-2 rounded-full transition ${i === current ? "bg-[var(--secondary)] w-6" : "bg-white/30"}`} aria-label={`Omdöme ${i + 1}`} />
              ))}
            </div>
            <button type="button" onClick={() => setCurrent((c) => (c + 1) % total)} className="rounded-full border border-white/20 p-2" aria-label="Nästa omdöme">→</button>
          </div>
        </div>
        ...
        <div className="mt-10 text-center">
          <a href={business.bookingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-full bg-[var(--secondary)] px-8 py-3 text-sm font-semibold text-[var(--dark)] transition hover:bg-[var(--secondary)]">
